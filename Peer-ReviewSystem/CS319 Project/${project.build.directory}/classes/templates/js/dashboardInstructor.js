...
function randomForm() {
    ...
    let section = parseInt(localStorage.getItem("currentSection"));
...
function getAllGroupsInCurrentSection() {
    // alert(parseInt(localStorage.getItem("currentSection));
    let section = parseInt(localStorage.getItem("currentSection"));
    let httpRequest = new XMLHttpRequest();
    httpRequest.overrideMimeType("application/json");
    httpRequest.open("GET", 'http://localhost:8080//SectionController/getGroupsOfSection?sectionNumber='+section);
    httpRequest.onload = function () {
        let groups = JSON.parse(httpRequest.responseText, function (key, value) {
...
function getAllSectionsOfInstructor() {
    let id = parseInt(localStorage.getItem("id"));
    alert("Ide of instructor is " + id);
    let httpRequest = new XMLHttpRequest();
    httpRequest.overrideMimeType("application/json");
    httpRequest.open("GET", 'http://localhost:8080//SectionController/getAllSectionsOfInstructor?instructorID='+id);
    httpRequest.onload = function () {
        let js = parse(httpRequest.responseText);
        alert(js);
        let sections = [];
        for(let i=0; i<1; i++) {
            sections.push(js["0"]);
